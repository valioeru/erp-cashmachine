...
const NAV = [
  // Emailul a stat o vreme aici, ca intrare de sine statatoare. A coborat sub
  // Management: inboxul intreg e unealta de condus firma, nu de operat in ea.
  // Emailurile care conteaza pentru munca de zi cu zi se vad oricum acolo unde
  // le e locul — pe fisa partenerului, a ofertei si a facturii.
  { href: "/crm", label: "Vânzări" },
  { href: "/marketing/contacte", label: "Marketing", zona: "/marketing" },
  { href: "/stocuri/ct-park", label: "Depozit", zona: "/depozit" },
  { href: "/productie", label: "Producție" },
  { href: "/procurement", label: "Procurement" },
  { href: "/financiar", label: "Financiar" },
  // Bugetul stă separat, la cererea lui Vali: nu e un raport pe care îl citești,
  // e un document la care lucrezi — și se deschide de zeci de ori într-o lună
  // de planificare. Sub „Rapoarte" ar fi fost cu două clicuri mai departe.
  { href: "/buget", label: "Buget", zona: "/buget" },
  // Dezvoltarea a stat o vreme DOAR sub Management. Dreptul de a scrie o cerere
  // il avea toata lumea (MEREU_PERMISE din lib/auth), dar intrarea din bara se
  // vedea doar daca omul avea voie la o zona de management — adica la nimeni in
  // afara de admin. Agentii aveau dreptul si n-aveau pe unde sa intre: pagina
  // exista, dar nu ducea niciun buton la ea.
  //
  // De-aia sta acum si pe cont propriu, la vedere pentru oricine e logat:
  // problema pe care o vede omul de la depozit sau agentul in fata clientului e
  // exact aia care trebuie sa ajunga la administrator, si nu are cum s-o scrie
  // daca nu gaseste unde. Filtrul de mai jos o arata tuturor fara nicio bifa,
  // fiindca poateAccesa("/dezvoltare") e adevarat pentru oricine e logat.
  { href: "/dezvoltare", label: "Dezvoltare" },
  //
  // Management strange la un loc ce tine de condus firma, nu de operat in ea:
  // tabloul de bord, rapoartele, configurarile si utilizatorii. Sunt patru
  // ZONE separate la drepturi (cine vede rapoartele nu vede si utilizatorii),
  // dar o singura intrare in bara de sus — de-aia „zone" e o lista: intrarea
  // se arata daca omul are voie la macar una dintre ele.
  // „zone" decide cine VEDE intrarea; „evidentiaza" decide doar cand se
  // aprinde. Cele doua nu-s acelasi lucru: /email e deschis oricui
  // (MEREU_PERMISE), deci pus in „zone" ar fi facut Managementul vizibil
  // pentru toata lumea. Asa, omul de la depozit intrat pe /email vede
  // intrarea aprinsa doar daca are oricum drept la ea.
  //
  // „/dezvoltare" NU mai e aici: are butonul lui, si s-ar fi aprins doua
  // deodata.
  {
    href: "/",
    label: "Management",
    zone: ["/", "/rapoarte", "/configurari", "/admin/utilizatori"],
    evidentiaza: ["/email"],
  },
];

// Calculatoarele stau sub o singură intrare de meniu („Calculator preț").
// Care anume se deschide se alege din lista derulantă, iar fiecare are pagina
// lui — deci se poate trimite un link direct la calculul potrivit.
const CALCULATOARE = [
  ["/calculator", "Preț pe categorie de produs"],
  ["/calculator/awb", "Plicuri AWB — C3 / C4 / C5 / C6"],
  ["/calculator/pungi", "Pungi curier — de la granulă la cutie"],
  ["/calculator/cutii", "Cutii carton — formate/lipite și autoformare"],
];

function selectorCalculator(activ) {
  const opt = CALCULATOARE.map(
    ([href, et]) => `<option value="${href}"${href === activ ? " selected" : ""}>${esc(et)}</option>`
  ).join("");
  return `<div class="calc-selector">
    <label class="field" style="margin:0;max-width:420px">
      <span>Calculator</span>
      <select onchange="if(this.value&&this.value!=location.pathname)location.href=this.value">${opt}</select>
    </label>
  </div>`;
}
...
function subnavCrm(activ, user) {
  ...
  const linkuri = [
    ...
    ["/crm/concurenta", "Prețurile concurenței"],
    ["/crm/emailuri", "Emailurile clienților mei"],
...
function subnavDepozit(activ, user) {
  return subnav(
    [
      ...
      ["/depozit/inventar", "Inventar fizic"],
...
// Management: tabloul de bord, rapoartele, configurarile si administrarea.
// Fiecare link se arata doar daca omul are voie la el — subnav() filtreaza.
function subnavManagement(activ, user) {
  return subnav(
    [
      ["/", "Dashboard"],
      ["/rapoarte", "Rapoarte"],
...
// Marketing: oamenii si ce le trimitem.
function subnavMarketing(activ, user) {
  return subnav(
    [
      ["/marketing/contacte", "Contacte"],
      ["/marketing/aniversari", "Aniversări"],
      ["/marketing/emailuri", "Emailuri trimise"],
    ],
    activ,
    user
  );
...
function subnavPentru(activ, user) {
  ...
  if (z === "/marketing") return subnavMarketing(activ, user);
  // Cele patru zone de management impart aceeasi subnavigatie.
  if (
    z === "/" || z === "/rapoarte" || z === "/configurari" || z === "/admin/utilizatori" ||
    z === "/email" || z === "/dezvoltare"
  ) {
    return subnavManagement(activ, user);
  }
  return "";
}
...
function layout({ title, active, body, flash, user }) {
  const navItems = NAV.filter((item) => {
    ...
    if (item.zone) return item.zone.some((z) => poateAccesa(user, z));
    return poateAccesa(user, item.zona || item.href);
  });
  const zonaActiva = sectiune(active);
  const navHtml = navItems
    .map((item) => {
      const activa = item.zone
        ? item.zone.concat(item.evidentiaza || []).includes(zonaActiva)
        : zonaActiva === (item.zona || item.href);
      return `<a href="${item.href}" class="navlink${activa ? " active" : ""}">${esc(item.label)}</a>`;
    })
    .join("");
...
(function () {
  function valoare(td) {
    // O celulă poate conține o listă derulantă (ex. reprezentantul din
    // registrul de comenzi). Acolo textContent le-ar da pe TOATE opțiunile,
    // deci toate rândurile ar arăta la fel. Luăm doar ce e ales.
...
module.exports = { esc, money, selectorCalculator, dataRo, dateleInText, avatar, layout, table, actionLinks, subnavCrm, subnavDepozit, subnavFinanciar, subnavProductie, subnavAdmin, subnavPentru, sectiune, NAV };
