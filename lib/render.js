...
const NAV = [
  ...
  { href: "/", label: "Management", zone: ["/", "/rapoarte", "/configurari", "/admin/utilizatori"] },
];
...
function subnavPentru(activ, user) {
  ...
  if (z === "/" || z === "/rapoarte" || z === "/configurari" || z === "/admin/utilizatori" || z === "/email") {
...
function layout({ title, active, body, flash, user }) {
  ...
  const navHtml = navItems
    .map((item) => {
      const activa = item.zone ? item.zone.includes(zonaActiva) : zonaActiva === (item.zona || item.href);
