...
function subnavCrm(activ, user) {
  ...
  return subnav(linkuri, activ);
}

// Depozitul: ce e pe stoc, ce a comandat lumea și ce cumpărăm.
function subnavWarehouse(activ) {
  return subnav(
    [
      ["/warehouse", "Comenzi deschise"],
      ["/stocuri", "Stocuri"],
      ["/depozite", "Depozite"],
      ["/warehouse/aprovizionare", "De aprovizionat"],
...
function subnavFinanciar(activ) {
  return subnav(
    [
      ["/financiar", "Sumar"],
      ["/banca", "Bancă"],
...
const SECTIUNI = [
  [["/warehouse", "/stocuri", "/depozite"], "/warehouse"],
  [["/financiar", "/banca", "/angajati", "/salarii", "/costuri", "/import"], "/financiar"],
...
function subnavPentru(activ, user) {
  ...
  if (z === "/warehouse") return subnavWarehouse(activ);
  if (z === "/financiar") return subnavFinanciar(activ);
  if (z === "/productie") return subnavProductie(activ);
  if (z === "/admin/utilizatori") return subnavAdmin(activ);
  return "";
}

// Producția: comenzile ei, cererile venite din depozit și materia primă
// pe care o cere înapoi de la depozit.
function subnavProductie(activ) {
  return subnav(
    [
...
function layout({ title, active, body, flash, user }) {
  ...
  const navHtml = navItems
    .map((item) => `<a href="${item.href}" class="navlink${zonaActiva === item.href ? " active" : ""}">${esc(item.label)}</a>`)
    .join("");
...
module.exports = { esc, money, layout, table, actionLinks, subnavCrm, subnavWarehouse, subnavFinanciar, subnavProductie, subnavAdmin, subnavPentru, sectiune, NAV };
