...
function parseFisier(filename, buffer) {
  // .xls binar (BIFF8) — formatul în care exportă SmartBill Cloud TOT.
  // Îl citim cu parserul propriu (lib/xls-biff.js), fără dependențe externe
  // și fără să cerem utilizatorului conversie manuală în CSV.
  const pareBinarXls = buffer && buffer.length > 8 && buffer.readUInt32LE(0) === 0xe011cfd0;
  if (pareBinarXls || (/\.xls$/i.test(filename) && pareBinarXls)) {
    try {
      return require("./xls-biff").parseXLS(buffer);
    } catch (e) {
      throw new Error("Nu am putut citi fișierul .xls: " + e.message);
    }
  }
  if (/\.xlsx$/i.test(filename)) return parseXLSX(buffer);
  // Unele exporturi „.xls" sunt de fapt HTML sau text tab-separat.
  const text = buffer.toString("utf8");
  if (/^\s*<(!doctype|html|table)/i.test(text)) return parseTabelHTML(text);
  return parseCSV(text);
}

// Tabel HTML salvat cu extensia .xls — îl citim minimal, fără dependențe.
function parseTabelHTML(html) {
  const randuri = [];
  const reRand = /<tr[^>]*>([\s\S]*?)<\/tr>/gi;
  const reCel = /<t[hd][^>]*>([\s\S]*?)<\/t[hd]>/gi;
  let m;
  while ((m = reRand.exec(html))) {
    const celule = [];
    let c;
    reCel.lastIndex = 0;
    ...
    if (celule.length) randuri.push(celule);
  }
  return randuri;
}
...
module.exports = { parseTabelHTML, xlsxDisponibil, parseXLSX, parseCSV, parseFisier, normalizeHeader, gasesteColoana, gasesteRandHeader, parseNumar, parseData };
