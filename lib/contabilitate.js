...
// --- ancora soldurilor iniţiale ----------------------------------------------
// Data ultimului sold iniţial preluat din balanţa Conta. Când există, TOATE
// interogările de balanţă exclud înregistrările "auto" de dinainte de (şi
// inclusiv) ancoră: efectul acelor documente e deja cuprins în soldurile
// preluate de la contabil, iar fără excludere fiecare factură istorică ar fi
// numărată de două ori (o dată în sold, o dată ca rulaj propriu).
async function ancoraSolduri() {
  const r = await db.prepare("SELECT MAX(data) AS d FROM inregistrari_contabile WHERE sursa = 'sold_initial'").get();
...
async function balanta(deLa, panaLa, doarCuMiscari = true) {
  const ancora = await ancoraSolduri();
  const filtruAncora = ancora ? "AND (i.sursa <> 'auto' OR i.data > ?)" : "";
  const argsAncora = ancora ? [ancora] : [];
  const randuri = await db
    .prepare(
      `SELECT i.cont,
       ...
       WHERE i.data <= ? ${filtruAncora}
       GROUP BY i.cont
       ORDER BY i.cont`
    )
    .all(deLa, deLa, deLa, panaLa, deLa, panaLa, panaLa, ...argsAncora);
...
async function fisaCont(cont, deLa, panaLa) {
  ...
  const initial = await db
    .prepare(`SELECT COALESCE(SUM(i.debit),0) AS d, COALESCE(SUM(i.credit),0) AS c FROM inregistrari_contabile i WHERE i.cont = ? AND i.data < ? ${filtruAncora}`)
    .get(cont, deLa, ...argsAncora);
  const miscari = await db
    .prepare(
      `SELECT i.*, p.nume AS partener_nume
       FROM inregistrari_contabile i
       LEFT JOIN parteneri p ON p.id = i.partener_id
       WHERE i.cont = ? AND i.data >= ? AND i.data <= ? ${filtruAncora}
       ORDER BY i.data ASC, i.id ASC
       LIMIT 500`
    )
    .all(cont, deLa, panaLa, ...argsAncora);
...
module.exports = { regenereaza, balanta, fisaCont, evolutieZilnica, asiguraPlanConturi, hartaConturi, ancoraSolduri, BANI, CONTURI_IMPLICITE };
