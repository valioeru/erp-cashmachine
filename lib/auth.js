...
async function utilizatorDinToken(token) {
  ...
  const user = await db
    .prepare("SELECT id, nume, email, rol, activ, poza, COALESCE(comision_procent,2) AS comision_procent, COALESCE(parola_temporara,0) AS parola_temporara FROM utilizatori WHERE id = ?")
    .get(s.utilizator_id);
