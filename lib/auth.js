...
const ZONE = [
  ...
  { cheie: "/crm", eticheta: "Vânzări", ghid: "crm" },
  { cheie: "/marketing", eticheta: "Marketing" },
  { cheie: "/depozit", eticheta: "Depozit", ghid: "depozit" },
  { cheie: "/productie", eticheta: "Producție", ghid: "productie" },
  { cheie: "/procurement", eticheta: "Procurement" },
...
const SECTIUNI = [
  [["/depozit", "/depozite", "/stocuri", "/warehouse"], "/depozit"],
...
// O pagină poate ține de mai multe intrări din meniu.
//
// Fișa unui partener e cazul limpede: pentru agent e CLIENTUL LUI, pentru
// financiar e furnizorul de plătit. Stă sub „Financiar" fiindcă acolo se
// aprinde butonul din bara de sus — dar dacă accesul s-ar da tot după asta,
// agenții cărora li s-a bifat „Vânzări" ar primi „Nu ai acces la această
// secțiune" exact pe clienții lor. (S-a și întâmplat.)
//
// Deci: pentru NAVIGARE contează o singură zonă, ca să nu se aprindă două
// butoane deodată; pentru ACCES contează toate — cine are oricare dintre ele
// poate deschide pagina.
const ZONE_IN_PLUS = [[["/parteneri", "/contacte"], ["/crm"]]];

function zoneleRutei(pathname) {
  const p = String(pathname || "/");
  const out = [sectiune(p)];
  for (const [prefixe, zone] of ZONE_IN_PLUS) {
    if (prefixe.some((x) => p === x || p.startsWith(x + "/"))) out.push(...zone);
  }
  return [...new Set(out)];
}
...
// Oricine logat poate cere o modificare in soft si poate comenta pe cererea
// altcuiva, indiferent de rol: problema pe care o vede omul de la depozit e
// exact aia care trebuie sa ajunga la administrator. Aprobarea ramane a
// administratorului, verificata separat in modul.
// Marketingul e al tuturor: oricine vorbeste cu un client are de adaugat un
// om nou in agenda, si nu are sens sa astepte o bifa pentru asta. Stergerea
// ramane a administratorului, verificata separat in modul.
// Preturile concurentei le poate scrie oricine: informatia de piata se afla
// la telefon, in fata clientului sau la targ, si se pierde daca omul trebuie
// sa astepte o bifa ca s-o scrie. Stergerea ramane a celui care a scris
// randul si a administratorului, verificata separat in modul. Rapoartele in
// sine stau sub zonele lor (/crm, /procurement, /rapoarte) si se vad dupa
// drepturile obisnuite — aici e doar scrierea.
const MEREU_PERMISE = ["/profil", "/logout", "/ghid", "/dezvoltare", "/marketing", "/concurenta", "/email"];
...
function poateAccesa(u, pathname) {
  ...
  if (alese.length) return zoneleRutei(p).some((z) => alese.includes(z));
