...
// Trimiterea e un scope separat de citire, dinadins: „send" nu poate citi
// nimic, iar „readonly" nu poate trimite nimic. Chiar dacă cineva ar greși
// configurarea, cele două nu se pot amesteca.
const SCOPE_GMAIL_TRIMITE = "https://www.googleapis.com/auth/gmail.send";
...
function explica(r, casuta) {
  const j = (r && r.json) || {};
  // Google are DOUĂ formate de eroare, și diferența ne-a costat o oră:
  //   - capătul de token:  { error: "unauthorized_client", error_description: "..." }
  //   - API-urile REST:    { error: { code: 403, message: "...", status: "...",
  //                                   errors: [{ reason: "storageQuotaExceeded" }] } }
  // Pe al doilea, String(j.error) dădea „[object Object]" și mesajul adevărat
  // se pierdea de tot. Acum se citesc amândouă.
  const obj = j.error && typeof j.error === "object" ? j.error : null;
  const e = obj ? String(obj.status || obj.code || "") : String(j.error || "");
  const d = obj ? String(obj.message || "") : String(j.error_description || "");
  const motiv =
    obj && Array.isArray(obj.errors) && obj.errors[0] ? String(obj.errors[0].reason || "") : "";

  // Erorile de Drive, traduse la ce trebuie făcut. Fiecare dintre ele m-a
  // costat un drum înapoi prin ghid, așa că spun exact unde se repară.
  if (motiv === "storageQuotaExceeded" || /storage quota/i.test(d))
    return `Folderul e într-un „My Drive", nu într-un Drive partajat. Fișierele urcate acolo ar fi ale contului tehnic, iar un cont tehnic are spațiu zero — de-aia refuză Google. Mută folderul într-un Drive partajat (Shared drive) și pune ID-ul lui nou în GOOGLE_DRIVE_FOLDER. Pasul 3 din ghid. (${motiv || d})`;
  if (motiv === "insufficientFilePermissions" || motiv === "forbidden" || (r && r.cod === 403 && /permission/i.test(d)))
    return `Contul tehnic vede folderul, dar n-are voie să scrie în el. Dă-i drept de Content manager (nu Viewer, nu Commenter) pe folder sau pe Drive-ul partajat. Pasul 3 din ghid. (${motiv || d})`;
  // Atenție la 404: Drive îl dă și când fișierul chiar nu există, ȘI când
  // există dar operația îi e refuzată — ca să nu dea de înțeles că există.
  // De-aia mesajul spune amândouă, în ordinea probabilității, în loc să
  // trimită omul sigur pe pista greșită.
  if (motiv === "notFound" || (r && r.cod === 404))
    return `Google zice „nu găsesc” — ceea ce înseamnă ori că id-ul chiar nu există, ori că există dar operația e refuzată (Drive dă 404 în loc de 403 ca să nu confirme existența). Dacă e vorba de folder, verifică să fie bucata de după /folders/ din adresa folderului, nu a Drive-ului. Dacă folderul se citește bine și doar scrierea sau ștergerea pică, atunci e rolul contului tehnic în Drive-ul partajat. Pasul 3 din ghid. (${d || motiv})`;
  if (motiv === "cannotAddParent" || motiv === "teamDriveFileLimitExceeded")
    return `Drive-ul partajat refuză fișierul: ${d}. Pasul 3 din ghid. (${motiv})`;
  if (motiv === "appNotAuthorizedToFile")
    return `Fișierul nu e vizibil pentru contul tehnic. Pasul 3 din ghid. (${motiv})`;
...
const tokenGmailTrimite = (casuta) => token(SCOPE_GMAIL_TRIMITE, casuta);
