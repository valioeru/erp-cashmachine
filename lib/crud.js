...
function registerCrud(router, opts) {
  ...
  router.get(path, async (ctx) => {
    ...
    const body = `
      ...
      ${table(
        ...
        rows.map((r) => [
          ...cols.map((c) => esc(c.render ? c.render(r) : r[c.key])),
          actionLinks([
    ...
    `;
    send(ctx.res, 200, layout({ title, active: path, body }));
  });

  router.get(`${path}/nou`, (ctx) => {
    const body = renderForm(null, false);
    send(ctx.res, 200, layout({ title: `${singular} nou`, active: path, body }));
  });
  ...
  router.get(`${path}/:id/editare`, async (ctx) => {
    ...
    if (!row) return send(ctx.res, 404, layout({ title: "Negăsit", active: path, body: "<p>Rând inexistent.</p>" }));
    const body = renderForm(row, true);
    send(ctx.res, 200, layout({ title: `Editare ${singular}`, active: path, body }));
  });
