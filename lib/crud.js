...
function registerCrud(router, opts) {
  // afterCreate(id, ctx): apelat după inserarea unui rând nou, cu id-ul lui.
  // Există ca modulele să poată lega ceva de rândul proaspăt creat (de pildă
  // alocarea clientului pe agentul care l-a introdus) fără să-și scrie fiecare
  // propria rută de adăugare.
  const { path, table: tbl, title, singular, fields, listColumns, extraBody, afterCreate } = opts;
  ...
  router.post(path, async (ctx) => {
    ...
    const ins = await db.prepare(`INSERT INTO ${tbl} (${cols.join(",")}) VALUES (${placeholders}) RETURNING id`).run(...values);
    if (afterCreate) {
      try {
        await afterCreate(ins.lastInsertRowid, ctx);
      } catch (e) {
        // Rândul e deja salvat; dacă pasul de după eșuează, nu pierdem munca
        // omului din formular pentru atât — se vede în loguri și se repară.
        console.error(`[crud] afterCreate ${tbl}: ${e.message}`);
      }
    }
