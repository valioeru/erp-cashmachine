...
// Ce e în folderul ăsta. Se folosește la trimiterea unui email: omul alege
// din listă ofertele și fișele tehnice pe care vrea să le atașeze, în loc să
// le descarce din Drive și să le urce înapoi din calculator.
//
// Se întorc și subfolderele, ca să se poată umbla prin ele — un folder de
// oferte se organizează pe furnizor sau pe an, nu stă plat la infinit.
async function listeaza(parinte, optiuni) {
  const o = optiuni || {};
  const token = await g.tokenDrive();
  const bucati = [`'${apostrof(parinte)}' in parents`, "trashed=false"];
  if (o.cauta) {
    // Drive nu are „contains" pe nume cu diacritice de încredere, dar merge
    // suficient de bine pe bucăți simple. Filtrul fin se face oricum în ERP.
    bucati.push(`name contains '${apostrof(String(o.cauta).slice(0, 60))}'`);
  }
  const q = bucati.join(" and ");
  const campuri = "files(id,name,mimeType,size,modifiedTime),nextPageToken";
  const r = await g.apel(
    "GET",
    `${FISIERE}?q=${encodeURIComponent(q)}&fields=${encodeURIComponent(campuri)}&pageSize=${Number(o.cate) || 200}` +
      `&orderBy=${encodeURIComponent("folder,modifiedTime desc")}&${COMUN}`,
    { token }
  );
  const fisiere = (r.json && r.json.files) || [];
  return fisiere.map((f) => ({
    id: f.id,
    nume: f.name,
    mime: f.mimeType,
    folder: f.mimeType === MIME_FOLDER,
    marime: Number(f.size) || 0,
    modificat: String(f.modifiedTime || "").slice(0, 10),
  }));
}

// Conținutul unui fișier, ca Buffer. Pentru atașarea la email.
//
// Documentele native Google (Docs, Sheets) n-au conținut de descărcat direct
// — se exportă. Aici le exportăm în PDF: o ofertă trimisă unui client se
// trimite ca PDF, nu ca link de Google Docs pe care s-ar putea să nu-l poată
// deschide.
const EXPORT_NATIV = {
...
async function descarca(id) {
  const token = await g.tokenDrive();
  const meta = await info(id);
  const nativ = EXPORT_NATIV[meta.mimeType];
  const adresa = nativ
    ? `${FISIERE}/${encodeURIComponent(id)}/export?mimeType=${encodeURIComponent(nativ[0])}&${COMUN}`
    : `${FISIERE}/${encodeURIComponent(id)}?alt=media&${COMUN}`;
  const r = await g.apel("GET", adresa, { token, binar: true });
  const continut = Buffer.isBuffer(r.corp) ? r.corp : Buffer.from(r.corp || "", "binary");
  let nume = String(meta.name || "fisier");
  if (nativ && !nume.toLowerCase().endsWith(nativ[1])) nume += nativ[1];
  return { nume, mime: nativ ? nativ[0] : meta.mimeType || "application/octet-stream", continut };
}
...
// Mută la coș, nu șterge definitiv.
//
// Într-un Drive partajat, ștergerea definitivă (DELETE) e rezervată rolului
// Manager. Un Content manager — cum e contul nostru tehnic, și cum trebuie să
// fie — primește refuz. Iar Drive răspunde la refuzul ăsta cu 404
// „File not found", nu cu 403, adică fix mesajul pe care-l dă și pentru un id
// inexistent. Ne-a costat o oră de căutat folderul greșit.
//
// Mutarea la coș merge pentru Content manager, e reversibilă, și Drive golește
// coșul singur după 30 de zile.
async function sterge(id) {
  const token = await g.tokenDrive();
  await g.apel("PATCH", `${FISIERE}/${encodeURIComponent(id)}?${COMUN}`, {
    token,
    json: { trashed: true },
  });
}
...
module.exports = { info, folder, cale, dupaMd5, urca, sterge, linkul, md5, curatNume, listeaza, descarca, reseteaza };
