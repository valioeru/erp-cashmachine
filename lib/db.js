...
-- Threaduri din inboxul office@ în care ultimul mesaj vine din exterior și
-- noi n-am mai răspuns. ERP-ul nu vede emailul, așa că lista o face agentul
-- de dimineață și o aduce prin „date/radar-raspunsuri.json", pe același drum
-- ca agenda și știrile.
--
-- „severitate" nu e prioritatea celui care scrie, ci cât ne costă tăcerea:
--   blocaj  — oprește operarea sau un cont (cel mai mare client, un curier);
--   calitate— reclamație de la un client, escaladează singură cât tace;
--   comercial— cerere de ofertă, comandă, termen de livrare;
--   info    — restul.
-- „stare" ține minte ce am făcut noi: nou / vazut / rezolvat. Rândurile
-- rezolvate rămân, ca să se vadă de la o zi la alta ce a fost închis.
CREATE TABLE IF NOT EXISTS radar_raspunsuri (
  id SERIAL PRIMARY KEY,
  cheie_thread TEXT NOT NULL UNIQUE,
  sectiune TEXT NOT NULL DEFAULT 'noi',
  ...
  pas_urmator TEXT,
  primit_la TEXT,
  necitit INTEGER NOT NULL DEFAULT 0,
  a_insistat INTEGER NOT NULL DEFAULT 0,
  link TEXT,
  partener_id INTEGER,
